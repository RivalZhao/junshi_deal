...
def get_url(driver, url, text_xpath, button_xpath, url_xpath, page):
    driver.get(url)
    ...
    driver.implicitly_wait(5)
    sleep(1)
    driver.find_element_by_xpath(text_xpath).send_keys(page) #第几页 改这里~~
    sleep(1)
...
def get_all_text_x(url_list):
    ...
    for i in range(len(url_list)):
        try:
            ...
            #使用正则筛选标题
            # pattern = re.compile('(中国|我|中方|解放军|国产)')
            # match = re.search(pattern, node[0].text)
            # if not match:
            #     continue
            # print('start load')
...
def save_picture(imgurl, num, extension_name):
    ...
    if match:
        ...
        print('no match no match no match no match no match no match')
    num = str(num)
...
if __name__=="__main__":
    ...
    for page in range(20, 0, -1):
        military_url_list = get_url(driver, 'http://mil.news.sina.com.cn/jssd/',
                        ...
                        page)
        military_url_list_sum.extend(military_url_list)


    #/html/body/div[4]/div[4]/div[1]/div/div[2]/div[2]/h3/a
    #/html/body/div[5]/div[4]/div[1]/div/div[2]/div[29]/h3/a

    #military_url_list_sum = ['https://mil.news.sina.com.cn/jssd/2018-10-15/doc-ifxeuwws4443837.shtml']
    #get_all_text(driver, military_url_list) 弃用 selenium速度慢
    get_all_text_x(military_url_list_sum)
